...
DB_PATH = 'database.db'
chat_messages = []  # Απλή λίστα στη μνήμη για chat (μπορεί να βάλεις βάση μετά)
...
def subjects():
    ...
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("SELECT subject_semester, subject_name FROM subjects")
    rows = c.fetchall()
    conn.close()

    subjects_by_semester = {}
    for semester, subject in rows:
        subjects_by_semester.setdefault(semester, []).append(subject)
...
# Νέο endpoint για λήψη όλων των μηνυμάτων (GET)
@app.route('/chat/messages')
def get_chat_messages():
    if 'username' not in session:
        return jsonify({'error': 'Δεν είστε συνδεδεμένοι'}), 401
    # Επιστρέφουμε λίστα dict για JSON
    return jsonify([
        {'username': username, 'message': message} for username, message in chat_messages
    ])

# Endpoint για αποστολή μηνύματος (POST)
